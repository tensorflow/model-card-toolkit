...
from model_card_toolkit.utils import tf_source as src
...
class CoreTensorflowTest(parameterized.TestCase, TfxTest):

  def setUp(self):
    super(CoreTensorflowTest, self).setUp()
  ...
  @mock.patch.object(
      tf_graphics_utils, 'annotate_dataset_feature_statistics_plots', autospec=True)
  @mock.patch.object(tf_graphics_utils, 'annotate_eval_result_plots', autospec=True)
  def test_scaffold_assets_with_store(self, mock_annotate_data_stats,
    ...
    mct = core.ModelCardToolkit(
        ...
        mlmd_source=src.MlmdSource(
            store=store, model_uri=tfx_testdata_utils.TFX_0_21_MODEL_URI))
