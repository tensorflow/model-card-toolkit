...
class TestDataUtilsTest(absltest.TestCase):
  ...
  def setUp(self):
    ...
    self.store = tfx_testdata_utils.get_tfx_pipeline_metadata_store(tmp_db_path)
    self.assertIsNotNone(self.store)

  def test_get_tfx_pipeline_metadata_store_model_uri_exists(self):
    self.assertNotEmpty(
        self.store.get_artifacts_by_uri(tfx_testdata_utils.TFX_0_21_MODEL_URI))
  ...
  def test_get_tfx_pipeline_metadata_store_metrics_artifact_exists(self):
    ...
    self.assertLen(metrics, len(tfx_testdata_utils.TFX_0_21_METRICS_ARTIFACT_IDS))
